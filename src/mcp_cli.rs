...
#[allow(dead_code)] // fields are read by the TOML-editing implementation in the full build
...
pub(crate) fn list_entries(_root: &Path) -> std::io::Result<Vec<crate::config::McpConfig>> {
    unimplemented!("MCP server list is not implemented in this worktree")
}

pub(crate) fn add_entry(_root: &Path, _entry: NewMcpEntry) -> std::io::Result<()> {
    unimplemented!("MCP server add is not implemented in this worktree")
}

pub(crate) fn remove_entry(_root: &Path, _name: &str) -> std::io::Result<()> {
    unimplemented!("MCP server remove is not implemented in this worktree")
}
